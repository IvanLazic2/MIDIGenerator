...
def main(args):
    ...
    if args.checkpoint_directory is None or args.checkpoint is None:
        logger.warning("Did not specify checkpoint. Using random weights.")
    else:
    ...
    logger.info(f"Tokenised prompt length: {start_tokens.shape[0]}")
    ...
    if start_tokens.shape[0] >= config.max_sequence_length:
        logger.warning("Tokenised prompt too big.")
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="file",
        help="Prompting mode. file/unconditional",
    )
    parser.add_argument(
        "--prompt_midi",
        type=str,
        default=None,
        help="Path to the MIDI prompt file",
    )
    parser.add_argument(
        "--prompt_midi_slice",
        type=int,
